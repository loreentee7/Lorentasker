...
import { LayoutDashboard, CheckSquare, CalendarDays, StickyNote, FolderKanban, BarChart3, Settings, Search, Plus, Bell, Clock3, Circle, CheckCircle2, MoreHorizontal, ArrowUpRight, Sparkles, X, Trash2, Download, Upload, RotateCw, Flag, Menu, ListTodo, Coffee, Code2, Rocket, Palette, DatabaseBackup, LogOut, LockKeyhole, UserPlus, ShieldCheck, Camera, Play, Pause, Volume2, Pencil, FolderPlus } from 'lucide-react';
import { AnimatePresence } from 'framer-motion';
import { BlurText, FadeUp, HoverLift, ScaleIn, ViewTransition } from './components/amicro';
import './styles.css';
...
const defaultProjects = [
  { id: 'szendex', name: 'SZENDEX', description: 'Producto principal', color: 'coral' },
  { id: 'core', name: 'Core', description: 'Plataforma y arquitectura', color: 'violet' },
  { id: 'producto', name: 'Producto', description: 'Discovery y planificación', color: 'blue' },
  { id: 'docs', name: 'Docs', description: 'Documentación técnica', color: 'green' },
  { id: 'personal', name: 'Personal', description: 'Organización personal', color: 'orange' }
];
...
function Root() {
  ...
  const updateUser = changes => { const updated = { ...session, ...changes }; localStorage.setItem('lt-users', JSON.stringify(readUsers().map(u => u.id === updated.id ? updated : u))); setSession(updated); };
  return session ? <WorkspaceApp key={session.id} user={session} onLogout={logout} onUpdateUser={updateUser}/> : <AuthScreen onAuthenticated={login}/>;
}
...
function WorkspaceApp({ user, onLogout, onUpdateUser }) {
  ...
  const [notifications, setNotifications] = useStored(store('notifications'), []);
  ...
  useEffect(() => {
    const todayPending = tasks.filter(t => !t.done && isToday(new Date(t.date)));
    setNotifications(old => {
      const retained = old.filter(n => n.type !== 'daily');
      return todayPending.length ? [{ id: `daily-${format(today,'yyyy-MM-dd')}`, type: 'daily', title: `${todayPending.length} tareas para hoy`, body: 'Tu planificación diaria está lista.', read: false, createdAt: new Date().toISOString() }, ...retained] : retained;
    });
    const notificationKey = store(`notified-${format(today,'yyyy-MM-dd')}`);
    if (todayPending.length && !localStorage.getItem(notificationKey)) { window.lorentasker?.showNotification({ title: 'Tu día en Lorentasker', body: `Tienes ${todayPending.length} tareas previstas para hoy.` }); localStorage.setItem(notificationKey, '1'); }
  }, [tasks.length]);
  useEffect(() => { const handler = e => { if ((e.metaKey || e.ctrlKey) && e.key.toLowerCase() === 'n') { e.preventDefault(); setModal('task'); } if ((e.metaKey || e.ctrlKey) && e.key.toLowerCase() === 'k') { e.preventDefault(); document.querySelector('.search input')?.focus(); } if (e.key === 'Escape') setModal(null); }; window.addEventListener('keydown', handler); return () => window.removeEventListener('keydown', handler); }, []);
  ...
  const saveTask = data => { if (data.id) setTasks(tasks.map(t => t.id === data.id ? { ...t, ...data } : t)); else addTask(data); setModal(null); setToast(data.id ? 'Tarea actualizada' : 'Tarea añadida a tu panel'); };
  const deleteProject = id => { const project = projects.find(p => p.id === id); if (!project || !window.confirm(`¿Eliminar el proyecto “${project.name}”? Sus tareas pasarán a Sin proyecto.`)) return; setTasks(tasks.map(t => t.project === project.name ? { ...t, project: 'Sin proyecto' } : t)); setProjects(projects.filter(p => p.id !== id)); setToast(`Proyecto “${project.name}” eliminado`); };
  const addProject = data => { setProjects([...projects, { id: crypto.randomUUID(), ...data }]); setModal(null); setToast('Proyecto creado'); };
  ...
  return <div className={`app accent-${accent} ${compact ? 'compact' : ''}`}>
    <aside className="sidebar">
      ...
      <div className="side-projects"><div className="side-label">PROYECTOS <button onClick={()=>setModal('project')} title="Nuevo proyecto"><Plus size={14}/></button></div>{projects.slice(0,5).map(p=><button key={p.id} onClick={()=>{setView('tasks');setQuery(p.name)}}><i className={p.color}/>{p.name}<span>{tasks.filter(t=>t.project===p.name&&!t.done).length}</span></button>)}</div>
      <div className="sidebar-bottom"><button onClick={() => setView('settings')} className={view === 'settings' ? 'active' : ''}><Settings size={18}/> Ajustes</button><div className="profile" onDoubleClick={()=>setModal('profile')}><div className={`avatar ${user.avatar?'has-photo':''}`}>{user.avatar?<img src={user.avatar}/>:user.name.split(/\s+/).map(x=>x[0]).join('').slice(0,2).toUpperCase()}</div><div><strong>{user.name}</strong><span>@{user.username} · Local</span></div><button className="profile-edit" title="Editar perfil" onClick={()=>setModal('profile')}><Pencil size={14}/></button><button className="logout" title="Cerrar sesión" onClick={onLogout}><LogOut size={16}/></button></div></div>
    </aside>

    <main>
      <header><button className="mobile"><Menu/></button><div className="search"><Search size={18}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar tareas, notas o proyectos..."/><kbd>⌘ K</kbd></div><div className="header-actions"><button className="icon" onClick={()=>setNotificationsOpen(!notificationsOpen)}><Bell size={19}/>{notifications.some(n=>!n.read)&&<i/>}</button><button className="primary" onClick={()=>setModal('task')}><Plus size={18}/> Nueva tarea <kbd>⌘ N</kbd></button></div>{notificationsOpen&&<NotificationCenter notifications={notifications} setNotifications={setNotifications} onClose={()=>setNotificationsOpen(false)}/>}</header>
      <section className="content"><AnimatePresence mode="wait"><ViewTransition viewKey={view}>
        {view === 'dashboard' && <Dashboard {...{tasks:filtered, toggleTask, today, week, open, done, focusMinutes, setView, setModal, notes, user}}/>}
        {view === 'tasks' && <Tasks tasks={filtered} toggleTask={toggleTask} setTasks={setTasks} onAdd={()=>setModal('task')} onEdit={task=>setModal({type:'task',task})}/>}
        {view === 'calendar' && <CalendarView tasks={filtered} toggleTask={toggleTask}/>} 
        {view === 'notes' && <Notes notes={notes} setNotes={setNotes}/>} 
        {view === 'projects' && <Projects tasks={tasks} projects={projects} onAdd={()=>setModal('project')} onDelete={deleteProject} onOpen={name=>{setQuery(name);setView('tasks')}}/>}
        {view === 'stats' && <Stats tasks={tasks} focusMinutes={focusMinutes}/>} 
        {view === 'settings' && <SettingsView {...{accent,setAccent,compact,setCompact,update,setUpdate,user,onLogout,onEditProfile:()=>setModal('profile'),tasks,notes,projects,setTasks,setNotes,setProjects,setToast}}/>}
      </ViewTransition></AnimatePresence></section>
    </main>
    {(modal === 'task' || modal?.type === 'task') && <TaskModal task={modal?.task} projects={projects} onClose={()=>setModal(null)} onSave={saveTask}/>}
    {modal === 'profile' && <ProfileModal user={user} onClose={()=>setModal(null)} onSave={data=>{onUpdateUser(data);setModal(null);setToast('Perfil actualizado')}}/>}
    {modal === 'project' && <ProjectModal onClose={()=>setModal(null)} onSave={addProject}/>}
    {modal === 'focus' && <FocusModal tasks={tasks.filter(t=>!t.done)} onClose={()=>setModal(null)}/>}
...
function Dashboard({tasks,toggleTask,today,week,open,done,focusMinutes,setView,setModal,notes,user}) {
  ...
  return <>
    <PageTitle eyebrow={format(today,"EEEE, d 'de' MMMM",{locale:es})} title={`Buenos días, ${user.name.split(' ')[0]}`} desc="Aquí tienes una vista clara de todo lo que importa hoy." action={<button className="ghost" onClick={()=>setModal('focus')}><Sparkles size={17}/> Modo focus</button>}/>
...
function Metric({icon:Icon,label,value,meta,color,progress}) { return <HoverLift><div className="metric"><div className={`metric-icon ${color}`}><Icon size={20}/></div><div><span>{label}</span><strong>{value}</strong><small>{meta}</small>{progress!==undefined&&<div className="tiny-progress"><i style={{width:`${Math.min(100,progress)}%`}}/></div>}</div></div></HoverLift> }
...
function TaskRow({task,onToggle,onDelete,onEdit}) { return <div className={`task-row ${task.done?'done':''}`}><button className="check" onClick={onToggle}>{task.done?<CheckCircle2/>:<Circle/>}</button><div className="task-copy"><strong>{task.title}</strong><span><i className={`dot ${task.project.toLowerCase()}`}/>{task.project} · <Clock3 size={12}/>{task.time}</span></div><em className={`priority ${task.priority.toLowerCase()}`}>{task.priority}</em>{onDelete&&<button className="delete" title="Eliminar" onClick={onDelete}><Trash2 size={16}/></button>}<button className="more" title="Editar" onClick={onEdit}><Pencil size={15}/></button></div> }

function Tasks({tasks,toggleTask,setTasks,onAdd,onEdit}) { const [filter,setFilter]=useState('Todas'); return <><PageTitle eyebrow="ORGANIZACIÓN" title="Mis tareas" desc="Captura, prioriza y termina lo importante." action={<button className="primary" onClick={onAdd}><Plus size={18}/> Nueva tarea</button>}/><div className="toolbar">{['Todas','Pendientes','Completadas','Alta'].map(x=><button className={filter===x?'active':''} onClick={()=>setFilter(x)} key={x}>{x}</button>)}</div><div className="card full-list"><CardHead title="Tareas" subtitle={`${tasks.length} en total`}/>{tasks.filter(t=>filter==='Todas'||filter==='Pendientes'&&!t.done||filter==='Completadas'&&t.done||filter==='Alta'&&t.priority==='Alta').map(t=><TaskRow key={t.id} task={t} onToggle={()=>toggleTask(t.id)} onDelete={()=>setTasks(tasks.filter(x=>x.id!==t.id))} onEdit={()=>onEdit(t)}/>)}</div></> }
...
function Projects({tasks,projects,onAdd,onDelete,onOpen}) { return <><PageTitle eyebrow="ESPACIOS DE TRABAJO" title="Proyectos" desc="El contexto de cada frente, sin mezclar prioridades." action={<button className="primary" onClick={onAdd}><FolderPlus size={17}/> Nuevo proyecto</button>}/><div className="project-grid">{projects.map(p=>{const ts=tasks.filter(t=>t.project===p.name);const complete=ts.filter(t=>t.done).length;return <div className="project-card card" key={p.id}><div className="project-actions"><button onClick={()=>onOpen(p.name)}>Abrir</button><button className="danger" title="Eliminar proyecto" onClick={()=>onDelete(p.id)}><Trash2 size={15}/></button></div><i className={p.color}><FolderKanban/></i><span>{ts.length} tareas</span><h2>{p.name}</h2><p>{p.description}</p><div className="progress"><i style={{width:`${ts.length?complete/ts.length*100:0}%`}}/></div><small>{complete} de {ts.length} completadas</small></div>})}</div></> }
...
function SettingsView({accent,setAccent,compact,setCompact,update,setUpdate,user,onLogout,onEditProfile,tasks,notes,projects,setTasks,setNotes,setProjects,setToast}) { const check=()=>{setUpdate({state:'checking',message:'Buscando actualizaciones…'});window.lorentasker?.checkUpdates()||setTimeout(()=>setUpdate({state:'dev',message:'Disponible al instalar la aplicación'}),800)}; const exportAll=async()=>{const path=await window.lorentasker?.exportData({version:1,user:{name:user.name,username:user.username},tasks,notes,projects,exportedAt:new Date().toISOString()});if(path)setToast('Copia de seguridad exportada')}; const importAll=async()=>{try{const data=await window.lorentasker?.importData();if(!data)return;if(Array.isArray(data.tasks))setTasks(data.tasks);if(Array.isArray(data.notes))setNotes(data.notes);if(Array.isArray(data.projects))setProjects(data.projects);setToast('Copia importada correctamente')}catch{setToast('El archivo no es una copia válida')}}; return <><PageTitle eyebrow="LORENTASKER" title="Ajustes" desc="Haz que tu espacio de trabajo se sienta realmente tuyo."/><div className="settings-grid"><div className="card settings-card"><h3><Palette/> Apariencia</h3><div className="setting"><div><strong>Color de acento</strong><span>Define la energía visual del dashboard</span></div><div className="swatches">{['coral','violet','blue','green'].map(x=><button className={`${x} ${accent===x?'active':''}`} onClick={()=>setAccent(x)} key={x}/>)}</div></div><div className="setting"><div><strong>Modo compacto</strong><span>Muestra más información por pantalla</span></div><button className={`switch ${compact?'on':''}`} onClick={()=>setCompact(!compact)}><i/></button></div></div><div className="card settings-card"><h3><DatabaseBackup/> Datos y cuenta</h3><div className="setting"><div><strong>{user.name}</strong><span>@{user.username} · cuenta local</span></div><button className="secondary" onClick={onEditProfile}><Camera size={14}/> Editar perfil</button></div><div className="setting"><div><strong>Copia de seguridad</strong><span>Exporta o recupera tareas, notas y proyectos</span></div><div className="setting-buttons"><button className="secondary" onClick={importAll}><Upload size={14}/> Importar</button><button className="secondary" onClick={exportAll}><Download size={14}/> Exportar</button></div></div><div className="setting"><div><strong>Sesión local</strong><span>Los datos están aislados para este usuario</span></div><button className="secondary" onClick={onLogout}><LogOut size={14}/> Salir</button></div></div><div className="card settings-card updates"><h3><RotateCw/> Actualizaciones</h3><p>Lorentasker descarga las nuevas versiones directamente desde GitHub Releases.</p><button className="primary" onClick={update.state==='ready'?()=>window.lorentasker?.installUpdate():check}>{update.state==='checking'?<RotateCw className="spin"/>:<Download/>}{update.state==='idle'?'Buscar actualizaciones':update.message}</button><small>Versión 1.3.0 · Motion by Amicro</small></div></div></> }

function NotificationCenter({notifications,setNotifications,onClose}) { const markAll=()=>setNotifications(notifications.map(n=>({...n,read:true}))); return <div className="notification-panel"><div className="notification-head"><div><span>CENTRO DE ACTIVIDAD</span><h3>Notificaciones</h3></div><button onClick={onClose}><X/></button></div>{notifications.length?<><div className="notification-list">{notifications.map(n=><button key={n.id} className={n.read?'read':''} onClick={()=>setNotifications(notifications.map(x=>x.id===n.id?{...x,read:true}:x))}><i/><div><strong>{n.title}</strong><span>{n.body}</span></div></button>)}</div><div className="notification-footer"><button onClick={markAll}>Marcar todo como leído</button><button onClick={()=>setNotifications([])}>Borrar todo</button></div></>:<div className="empty-notifications"><Bell/><strong>Todo al día</strong><span>No tienes notificaciones pendientes.</span></div>}</div> }

function ProfileModal({user,onClose,onSave}) { const [name,setName]=useState(user.name);const [avatar,setAvatar]=useState(user.avatar||'');const choose=async()=>{const selected=await window.lorentasker?.pickAvatar();if(selected)setAvatar(selected)};return <div className="modal-backdrop"><form className="modal profile-modal" onSubmit={e=>{e.preventDefault();if(name.trim())onSave({name:name.trim(),avatar})}}><div className="modal-head"><div><span>PERFIL LOCAL</span><h2>Personaliza tu identidad</h2></div><button type="button" onClick={onClose}><X/></button></div><button type="button" className="avatar-picker" onClick={choose}>{avatar?<img src={avatar}/>:<Camera/>}<span>Cambiar foto</span></button><label>Nombre visible<input value={name} onChange={e=>setName(e.target.value)}/></label><label>Usuario<input value={user.username} disabled/></label><div className="modal-actions"><button type="button" className="secondary" onClick={()=>setAvatar('')}>Quitar foto</button><button className="primary">Guardar perfil</button></div></form></div> }

function ProjectModal({onClose,onSave}) { const [name,setName]=useState('');const [description,setDescription]=useState('');const [color,setColor]=useState('coral');return <div className="modal-backdrop"><form className="modal" onSubmit={e=>{e.preventDefault();if(name.trim())onSave({name:name.trim(),description:description.trim()||'Proyecto personal',color})}}><div className="modal-head"><div><span>NUEVO PROYECTO</span><h2>Crea un espacio de trabajo</h2></div><button type="button" onClick={onClose}><X/></button></div><label>Nombre<input autoFocus value={name} onChange={e=>setName(e.target.value)} placeholder="Ej. Web corporativa"/></label><label>Descripción<input value={description} onChange={e=>setDescription(e.target.value)} placeholder="Objetivo principal del proyecto"/></label><label>Color<div className="swatches project-swatches">{['coral','violet','blue','green'].map(x=><button type="button" className={`${x} ${color===x?'active':''}`} onClick={()=>setColor(x)} key={x}/>)}</div></label><div className="modal-actions"><button type="button" className="secondary" onClick={onClose}>Cancelar</button><button className="primary"><FolderPlus/> Crear proyecto</button></div></form></div> }

function FocusModal({tasks,onClose}) { const [task,setTask]=useState(tasks[0]?.id||'');const [seconds,setSeconds]=useState(25*60);const [running,setRunning]=useState(false);useEffect(()=>{if(!running)return;const tick=setInterval(()=>setSeconds(s=>{if(s<=1){clearInterval(tick);setRunning(false);window.lorentasker?.showNotification({title:'Sesión completada',body:'Buen trabajo. Tómate un descanso antes del siguiente bloque.'});return 0}return s-1}),1000);return()=>clearInterval(tick)},[running]);const mins=String(Math.floor(seconds/60)).padStart(2,'0'),secs=String(seconds%60).padStart(2,'0');return <div className="modal-backdrop"><div className="focus-modal"><button className="focus-close" onClick={onClose}><X/></button><span>MODO FOCUS</span><h2>{mins}:{secs}</h2><select value={task} onChange={e=>setTask(e.target.value)}>{tasks.map(t=><option value={t.id} key={t.id}>{t.title}</option>)}</select><div className="focus-controls"><button className="primary" onClick={()=>setRunning(!running)}>{running?<Pause/>:<Play/>}{running?'Pausar':'Empezar'}</button><button className="secondary" onClick={()=>{setRunning(false);setSeconds(25*60)}}>Reiniciar</button></div><div className="focus-presets">{[15,25,45,60].map(m=><button onClick={()=>{setRunning(false);setSeconds(m*60)}} key={m}>{m} min</button>)}</div></div></div> }

function TaskModal({task,projects,onClose,onSave}) { const [title,setTitle]=useState(task?.title||'');const [project,setProject]=useState(task?.project||projects[0]?.name||'Sin proyecto');const [priority,setPriority]=useState(task?.priority||'Media');const [date,setDate]=useState(format(task?.date?new Date(task.date):new Date(),'yyyy-MM-dd'));const [time,setTime]=useState(task?.time||'09:00');const [estimate,setEstimate]=useState(task?.estimate||30);const submit=e=>{e.preventDefault();if(title.trim())onSave({...(task||{}),title:title.trim(),project,priority,date:new Date(`${date}T12:00:00`).toISOString(),time,estimate:Number(estimate)})};return <div className="modal-backdrop" onMouseDown={e=>e.target===e.currentTarget&&onClose()}><form className="modal" onSubmit={submit}><div className="modal-head"><div><span>{task?'EDITAR TAREA':'NUEVA TAREA'}</span><h2>{task?'Actualiza los detalles':'¿Qué hay que hacer?'}</h2></div><button type="button" onClick={onClose}><X/></button></div><label>Título<input autoFocus value={title} onChange={e=>setTitle(e.target.value)} placeholder="Ej. Revisar implementación del login"/></label><div className="form-row"><label>Proyecto<select value={project} onChange={e=>setProject(e.target.value)}>{projects.map(p=><option key={p.id}>{p.name}</option>)}<option>Sin proyecto</option></select></label><label>Prioridad<select value={priority} onChange={e=>setPriority(e.target.value)}><option>Alta</option><option>Media</option><option>Baja</option></select></label></div><div className="form-row"><label>Fecha<input type="date" value={date} onChange={e=>setDate(e.target.value)}/></label><label>Hora<input type="time" value={time} onChange={e=>setTime(e.target.value)}/></label></div><label>Estimación (minutos)<input type="number" min="5" step="5" value={estimate} onChange={e=>setEstimate(e.target.value)}/></label><div className="modal-actions"><button type="button" className="secondary" onClick={onClose}>Cancelar</button><button className="primary" type="submit">{task?<Pencil/>:<Plus/>} {task?'Guardar cambios':'Crear tarea'}</button></div></form></div>}
